...
const IndexPage = () => {
  ...
  return (
    <>
      <Navbar />
      <Center minHeight="calc(100vh - 50px)" bg="gray.100" flexDirection="column">
        ...
        <Box width="300px" mb={4} mr={4}>
          ...
          >
            {zonas.map((zona) => (
              <option key={zona.id} value={zona.id} style={{ color: 'black' }}>
                {zona.nombre}
              </option>
            ))}
          </Select>
          ...
          <Box width="300px" mb={4}>
            ...
            >
              {subzonas.map((subzona) => (
                <option key={subzona.id} value={subzona.id} style={{ color: 'black' }}>
                  {subzona.nombre}
                </option>
              ))}
            </Select>
